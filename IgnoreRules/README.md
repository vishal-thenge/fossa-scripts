...
category: The category can be "licensing" or "vulnerability".
token: The FOSSA API key.
...
Sends a GET request to the FOSSA API to fetch exceptions (ignore rules) based on the specified category.
Writes the fetched data into a CSV file named IgnoreRules.csv.
CSV Fields
...
Sample Output
Upon successful execution, the script will print: Created IgnoreRules.csv successfully.
